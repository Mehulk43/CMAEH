...
# from inference import conmh_inference
...
def cifar_dataset(config):
    ...
    return train_loader, test_loader, database_loader, \
           train_index.shape[0], test_index.shape[0], database_index.shape[0],test_dataset,database_dataset
...
def compute_result(dataloader, net, device):
    ...
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        _, _, _, hash_code = net(img.to(device),mask_ratio)
...
def CalcTopMapWithPR(qB, queryL, rB, retrievalL, topk):
    ...
    cum_recall = np.mean(recall, 0)
     # Calculate and print mean precision and recall
    mean_prec = np.mean(prec)
    mean_recall = np.mean(recall)
    # print("Mean Precision:\n", mean_prec)
    # print("Mean Recall:\n", mean_recall)
    end_time = time.time()
    retireve_time = end_time-start_time
    print("retrieval time is ", retireve_time)
    return topkmap, cum_prec, cum_recall,mean_prec,mean_recall
...
def validate(config, Best_mAP, test_loader, dataset_loader, net, bit, epoch, num_dataset):
    ...
    if "pr_curve_path" not in  config:
        ...
        mAP, cum_prec, cum_recall,mean_prec,mean_recall = CalcTopMapWithPR(tst_binary.numpy(), tst_label.numpy(),
    ...
    print(f"{config['info']} epoch:{epoch + 1} bit:{bit} dataset:{config['dataset']} MAP:{mAP} Best MAP: {Best_mAP} Precision: {mean_prec} Recall: {mean_recall}")
    print(config)
    return Best_mAP
